...
const Decryption = () => {
  ...
  const handleDecrypt = () => {
    try {
      const [d, n] = privateKey.slice(1, -1).split(', ').map(Number);
      const decrypted = encryptedMessage
        .split(' ')
        .map((num) => {
          const decryptedCharCode = modularExponentiation(Number(num), d, n); // Use modular exponentiation
          return String.fromCharCode(decryptedCharCode);
        })
        .join('');
      setDecryptedMessage(decrypted); // Update state
    } catch (error) {
      alert('Invalid private key or encrypted message.');
    }
  };
  ...
  return (
    <section className="prsection">
      ...
      <div className="home">
        <h1 className="form-title">Decryption</h1>

        {/* Encrypted Message Input */}
        <h2>Encrypted Message</h2>
        <textarea value={encryptedMessage} onChange={(e) => setEncryptedMessage(e.target.value)} placeholder="Enter the encrypted message" className="form-textbox" />

        {/* Private Key Input */}
        <h2>Private Key</h2>
        <textarea value={privateKey} onChange={(e) => setPrivateKey(e.target.value)} placeholder="Enter the private key (d, n)" className="form-textbox" />

        {/* Decrypt Button */}
        <button onClick={handleDecrypt} className="form-submit">Decrypt</button>

        {/* Display Decrypted Message */}
        {decryptedMessage && (
          <div>
            <h3>Decrypted Message:</h3>
            <textarea value={decryptedMessage} readOnly className="form-input read-only" />
          </div>
        )}

        {/* Back Button */}
        <button onClick={() => navigate('/')} className="form-submit">Back</button>
